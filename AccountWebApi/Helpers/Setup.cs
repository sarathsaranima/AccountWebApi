...
namespace AccountWebApi.Helpers
{
    ...
    {
        /// <summary>
        /// This method deletes and recreates the database file everytime the 
            ...
            using (var dbContext = new AccountDbContext())
            {
                ...
                if (!dbContext.Accounts.Any())
                {
                    var path = Path.Combine(Directory.GetCurrentDirectory(),
                        @"Helpers\Account.json");
                    if (File.Exists(path))
                    {
                        string jsonTxt = File.ReadAllText(path);
                        var accounts = Newtonsoft.Json.JsonConvert.DeserializeObject
                            <IEnumerable<Account>>(jsonTxt);
                ...
                {
                    var path = Path.Combine(Directory.GetCurrentDirectory(),
                        @"Helpers\Transaction.json");
                    if (File.Exists(path))
                    {
                        string jsonTxt = File.ReadAllText(path);
                        var transactions = Newtonsoft.Json.JsonConvert.DeserializeObject
                            <IEnumerable<AccountTransaction>>(jsonTxt);
